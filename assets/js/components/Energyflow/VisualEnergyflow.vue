<template>
	<div ref="container" class="visual-energyflow">
		<div class="node-container">
			<!-- Top Row -->
			<!-- Top Row -->
			<div v-if="pvConfigured || pvProduction > 0" class="row top-row">
				<div ref="pvNode" class="node pv-node" :class="{ active: pvProduction > 0 }">
			...
			<div class="row center-row">
				<div
					v-if="gridConfigured || gridImport > 0 || gridExport > 0"
					ref="gridNode"
					class="node grid-node"
					:class="{ active: gridImport > 0 || gridExport > 0 }"
				>
					<div class="icon">
					...
					<div class="value">
						...
						<span v-else-if="gridExport > 0"
							>{{ fmtW(gridExport, powerUnit) }} Out</span
						>
						<span v-else>0 W</span>
					</div>
				</div>

				<div ref="homeNode" class="node home-node active">
				...
				<div
					...
					ref="vehicleNode"
					class="node vehicle-node"
					:class="{ active: loadpointsPower > 0 }"
				>
			...
			<!-- Bottom Row -->
			<div v-if="batteryConfigured" class="row bottom-row">
				<div
					ref="batteryNode"
					class="node battery-node"
					:class="{ active: batteryPower !== 0 }"
				>
					<div class="icon"><BatteryIcon :soc="batterySoc" /></div>
					<div class="label">Battery ({{ fmtPercentage(batterySoc) }})</div>
					<div class="value">
						<span v-if="batteryDischarge > 0"
							>{{ fmtW(batteryDischarge, powerUnit) }} Out</span
						>
						<span v-else-if="batteryCharge > 0"
							>{{ fmtW(batteryCharge, powerUnit) }} In</span
						>
						<span v-else>0 W</span>
					</div>
				</div>
		...
		<svg v-if="paths.gridToHome" class="flow-lines" width="100%" height="100%">
			<defs>
				<marker
					id="arrow-grid-in"
					viewBox="0 0 10 10"
			...
			<g v-if="batteryConfigured && paths.homeToBattery">
				<path
					...
					class="flow-path"
					:class="{
						'battery-charge': batteryCharge > 0,
						'battery-discharge': batteryDischarge > 0,
					}"
					:marker-end="
						batteryCharge > 0
							? 'url(#arrow-battery-charge)'
							: batteryDischarge > 0
								? 'url(#arrow-battery-discharge)'
								: ''
					"
				/>
...
export default defineComponent({
	...
	methods: {
		...
		calculatePaths() {
			...
			// Grid
			let grid: any = (this.$refs as any)["gridNode"];
			if (Array.isArray(grid)) grid = grid[0];
			if (grid && "$el" in grid) grid = (grid as any).$el;
			...
			if (grid && grid instanceof HTMLElement) {
				...
				this.paths.gridToHome = `M ${p1.x} ${p1.y} C ${p1.x + 50} ${p1.y}, ${p2.x - 50} ${p2.y}, ${p2.x} ${p2.y}`;
				this.paths.homeToGrid = `M ${p2.x} ${p2.y} C ${p2.x - 50} ${p2.y}, ${p1.x + 50} ${p1.y}, ${p1.x} ${p1.y}`;
			}

			// PV
			let pv: any = (this.$refs as any)["pvNode"];
			if (Array.isArray(pv)) pv = pv[0];
			if (pv && "$el" in pv) pv = (pv as any).$el;
			...
			if (pv && pv instanceof HTMLElement) {
				...
				this.paths.pvToHome = `M ${p1.x} ${p1.y} C ${p1.x} ${p1.y + 50}, ${p2.x - 50} ${p2.y}, ${p2.x} ${p2.y}`;
			}

			// Battery
			let battery: any = (this.$refs as any)["batteryNode"];
			if (Array.isArray(battery)) battery = battery[0];
			if (battery && "$el" in battery) battery = (battery as any).$el;
			...
			if (battery && battery instanceof HTMLElement) {
				...
				const p2 = {
					x: cBattery.x - Math.cos(angle) * r,
					y: cBattery.y - Math.sin(angle) * r,
				};

				this.paths.homeToBattery = `M ${p1.x} ${p1.y} C ${p1.x - 50} ${p1.y}, ${p2.x} ${p2.y - 50}, ${p2.x} ${p2.y}`;
				this.paths.batteryToHome = `M ${p2.x} ${p2.y} C ${p2.x} ${p2.y - 50}, ${p1.x - 50} ${p1.y}, ${p1.x} ${p1.y}`;
			}

			// Vehicle
			let vehicle: any = (this.$refs as any)["vehicleNode"];
			if (Array.isArray(vehicle)) vehicle = vehicle[0];
			if (vehicle && "$el" in vehicle) vehicle = (vehicle as any).$el;
			...
			if (vehicle && vehicle instanceof HTMLElement) {
				...
				const p1 = { x: cHome.x + Math.cos(angle) * r, y: cHome.y + Math.sin(angle) * r };
				const p2 = {
					x: cVehicle.x - Math.cos(angle) * r,
					y: cVehicle.y - Math.sin(angle) * r,
				};

				this.paths.homeToVehicle = `M ${p1.x} ${p1.y} C ${p1.x + 50} ${p1.y}, ${p2.x - 50} ${p2.y}, ${p2.x} ${p2.y}`;
			}
		},
	},
});
...
.grid-node.active {
	border-color: var(--evcc-grid);
}
.pv-node.active {
	border-color: var(--evcc-pv);
}
.home-node.active {
	border-color: var(--evcc-gray);
}
.battery-node.active {
	border-color: var(--evcc-battery);
}
.vehicle-node.active {
	border-color: var(--evcc-export);
}
...
.flow-path.battery-charge {
	stroke: var(--evcc-battery);
	opacity: 0.9;
	stroke-dasharray: none;
	stroke-width: 5;
}
.flow-path.battery-discharge {
	stroke: var(--evcc-battery);
	opacity: 0.9;
	stroke-dasharray: none;
	stroke-width: 5;
}
.flow-path.vehicle-in {
	stroke: var(--evcc-export);
	opacity: 0.9;
	stroke-dasharray: none;
	stroke-width: 5;
